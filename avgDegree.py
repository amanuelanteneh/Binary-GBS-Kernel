...
for dataset in datasetNames:
  ...
  totalNodes = sum([ g.get_adjacency_matrix().shape[0] for g in graphs ])
  totalEdges = sum([ len(g.get_edges())/2 for g in graphs ])
  print("Avg degree for", dataset, "=", totalEdges/totalNodes)
...
for n in range(maxPhotons+1): # generate all binary strings of length M with n ones
